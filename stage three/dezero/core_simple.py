...
class Variable:
    ...
    def backward(self, retain_grad=False):
        # return_grad参数表示是否需要保存计算图中中间节点的梯度值，如果为false，则当这个节点没有用了之后，梯度会置空为None，用于优化内存
        if self.grad is None:
            '''
            用来给反向传播的最基础的导数：d(loss)/d(loss)赋初始值1
            用np.ones_like而不是np.array(1.0)是为了保证与self.data数据类型一致，要么都是32float，要么都是64float
            '''
